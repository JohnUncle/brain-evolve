 ...
 */
/**
 * Generate skill content using Claude Code.
 * @param {Array} rules - Array of positive pattern rules
 * @param {string} projectType - Project type
 * @param {string} slug - Skill slug
 * @param {Array} memories - User memories
 * @param {Array} sessionNarratives - Session narratives
 * @param {string} description - Skill description
 * @param {Array} antiPatterns - Array of anti-pattern rules (source='anti_pattern')
 */
function generateSkillContent(rules, projectType, slug, memories, sessionNarratives, description, antiPatterns) {
  ...
  // P4: Build anti-patterns description
  let antiPatternsDesc = '';
  if (antiPatterns && antiPatterns.length > 0) {
    antiPatternsDesc = '\nANTI-PATTERNS (things the user consistently does WRONG or wants to AVOID):\n';
    antiPatternsDesc += antiPatterns.map((r, i) =>
      `${i + 1}. ${r.content}`
    ).join('\n');
  }
...
This user has worked on a "${projectType}" project for ${sessionsObserved} sessions. From observing their behavior, these ${rules.length} positive rules emerged:

OBSERVED RULES (what the user consistently does RIGHT):
${rulesDesc}
${antiPatternsDesc}
${memoryContext}
${narrativeContext}
...
4. A "## What NOT to do" section:
   ${antiPatterns && antiPatterns.length > 0
     ? 'Explicitly list the anti-patterns observed. These are things the user does WRONG that Claude should help avoid. Format as clear warnings.'
     : 'Infer anti-patterns from corrections and observations. Things Claude should avoid.'}
   ${antiPatterns && antiPatterns.length > 0 ? `\n\nKnown anti-patterns to include:\n${antiPatterns.map(r => `- ${r.content}`).join('\n')}` : ''}
