...
function savePendingAndSpawn(projectPath, tempFile, preReadRecords) {
  try {
    ...
    // brain-evolve: detect cognitive patterns from this session
    let cognitiveSignals = [];
    try {
      const cognitiveModel = require(path.join(LEARNING_ROOT, 'cognitiveModel'));
      const userTranscript = observations.map(o => o.input || '').join(' ');
      cognitiveSignals = cognitiveModel.detectPatterns(observations, userTranscript);
      if (cognitiveSignals.length > 0) {
        try {
          fs.appendFileSync(
            path.join(LEARNING_ROOT, 'data', 'process.log'),
            new Date().toISOString() + ` [cognitive] detected ${cognitiveSignals.length} patterns: ${cognitiveSignals.map(p => p.pattern_name).join(', ')}\n`,
            'utf8'
          );
        } catch {}
      }
    ...
    const pending = {
      ...
      existingActiveRules,
      handWrittenContent,
      cognitiveSignals,  // brain-evolve: behavioral pattern detection results
    };
    ...
    return { spawned: true, memories: newMemories.length, observations: observations.length, cognitive: cognitiveSignals.length, pid: child.pid };
  } catch (err) {
    try {
