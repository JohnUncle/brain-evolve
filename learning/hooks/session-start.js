    ...
    try {
      ...
      try {
        ...
        if (activeRules.length > 0) {
          sections.push(`[brain-evolve] ${activeRules.length} active rules (see CLAUDE.md auto-learned section)`);
        }

        // brain-evolve: inject cognitive model patterns
        try {
          const cognitiveModel = require(path.join(LEARNING_ROOT, 'cognitiveModel'));
          const decay = require(path.join(LEARNING_ROOT, 'decay_scorer'));
          const rulesWithData = activeRules.map(r => ({
            ...r,
            confidence: decay.calculateConfidence(r),
          }));
          const cognitiveContext = cognitiveModel.generateSessionContext(rulesWithData);
          if (cognitiveContext) {
            sections.push(cognitiveContext);
          }

          // Also inject decay scan results (rules at VERIFY/REVALIDATE)
          const decayScan = ruleEngine.applyDecayScan(projectPath);
          const attentionNeeded = decayScan.filter(r => r.level === 'VERIFY' || r.level === 'REVALIDATE');
          if (attentionNeeded.length > 0) {
            sections.push('[brain-evolve] Decay Alert');
            for (const r of attentionNeeded) {
              const icon = r.level === 'REVALIDATE' ? '🔴' : '🟡';
              sections.push(`  ${icon} ${(r.content || '').slice(0, 60)} (${r.knowledge_type}, ${(r.confidence * 100).toFixed(0)}%, ${r.days_since_confirm}d)`);
            }
          }
        } catch (e) {
          // Cognitive model injection is optional — don't fail the hook
        }
      } catch {}
      ...
      try {
        ...
        if (conflicts.length > 0) {
          sections.push('⚠️ [brain-evolve Conflict Alert]');
      ...
      try {
        ...
        if (fs.existsSync(hintsPath)) {
          ...
          if (unshown.length > 0) {
            sections.push('[brain-evolve] Skill Update');
        ...
        if (fs.existsSync(xpPath)) {
          ...
          if (patterns.length > 0) {
            ...
            if (suggestions.length > 0) {
              sections.push(`[brain-evolve] Cross-project patterns available (${currentProjectType})`);
